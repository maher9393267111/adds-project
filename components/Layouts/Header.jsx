...
const Header = () => {
    ...
    const request = `${apiUrl}/Account/getuser/${userInfo?.id}`;
    ...
    // const { data, error } = useSWR([`${apiUrl}/Account/getuser/${userInfo?.id}` ,userInfo?.token] ,fetchwithToken );
                ...
                <div className="relative flex w-full items-center bg-white px-5 py-2.5 dark:bg-black">
                    <div className="horizontal-logo flex items-center justify-between ltr:mr-2 rtl:ml-2 lg:hidden">
                        <Link href="/" className="main-logo flex shrink-0 items-center">
                            <img
                                className="inline w-14 object-cover ltr:-ml-1 rtl:-mr-1"
                            

                                 src="/assets/project-image/get.png"
                                // "/assets/images/logo.svg"

                                alt="logo"
                            />
                            <span className="hidden align-middle text-2xl  font-semibold  transition-all duration-300 ltr:ml-1.5 rtl:mr-1.5 dark:text-white-light md:inline">easydeal</span>
                        </Link>
                        <button
                            type="button"
                    ...
                    <div className="flex justify-between items-center space-x-1.5 ltr:ml-auto rtl:mr-auto rtl:space-x-reverse dark:text-[#d0d2d6] sm:flex-1 ltr:sm:ml-0 sm:rtl:mr-0 lg:space-x-2">
                        ...
                        <div className="dropdown   flex shrink-0  justify-end relative    md:left-[93%] left-[7%] sm:left-[70%]">
                            <Dropdown
                                offset={[0, 8]}
                                placement={`${isRtl ? 'bottom-start' : 'bottom-end'}`}
                                btnClassName="relative group block"
                                button={<img className="h-9 w-9 rounded-full object-cover saturate-50 group-hover:saturate-100" 
                                src={`${userDetails?.photoUrl  ? userDetails?.photoUrl : defaultUserImage}`}
                            ...
                            >
                                <ul className="w-[230px] !py-0 font-semibold text-dark dark:text-white-dark dark:text-white-light/90">
                                        ...
                                        <div className="flex items-center px-4 py-4">
                                            ...
                                            <div className="ltr:pl-4 rtl:pr-4 truncate">
                                                <h4 className="text-base">
                                                    {userInfo?.username}
                                                    <span className="rounded bg-success-light px-1 text-xs text-success ltr:ml-2 rtl:ml-2">{userDetails?.role}</span>
                                                </h4>
                                    ...
                                    <li>
                                        <Link href={`/usersAds/userDetails/${userDetails?.id}`}
                                        // "/users/profile"
                                    ...
                                    <li onClick={logoutFunction} className="border-t border-white-light dark:border-white-light/10 !py-3 text-danger">
                                        <p className=" flex gap-2 ml-6 cursor-pointer">
